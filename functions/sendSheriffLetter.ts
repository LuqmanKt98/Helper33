...
Deno.serve(async (req) => {
  try {
    ...
    const emailBody = `
New Letter to Santa Clara County Sheriff's Office regarding Dr. Yuriy Dobry
...
Time: ${new Date().toISOString()}
    ...
    try {
      await base44.integrations.Core.SendEmail({
        ...
        subject: `Sheriff Letter: Dr. Yuriy Dobry Investigation - ${fullName}`,
        body: emailBody
      });
    } catch (emailError) {
    ...
    console.error('Sheriff letter submission error:', error);
    return Response.json(
