...
export default function RecommendedEvents({ limit = 3 }) {
  ...
  const { data: recommendedEvents = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // From wellness entries
      const wellnessEntries = await base44.entities.WellnessEntry.filter({}, '-created_date', 10);
      ...
      const journalEntries = await base44.entities.UserJournalEntry.filter({}, '-created_date', 10);
      if (journalEntries.some(e => e.journal_type === 'heart_shift')) {
        userInterests.add('grief_support');
      }
      
      // From completed challenges
      const completedChallenges = await base44.entities.ChallengeParticipant.filter({
        status: 'completed'
      });
      ...
      // From past RSVPs
      const pastRSVPs = await base44.entities.EventRSVP.filter({});
      const pastEventIds = pastRSVPs.map(r => r.event_id);
      
      if (pastEventIds.length > 0) {
        const pastEvents = await base44.entities.CommunityEvent.filter({});
        pastEvents
          .filter(e => pastEventIds.includes(e.id))
          .forEach(e => userInterests.add(e.category));
      }

      // Get all upcoming published events
      const allEvents = await base44.entities.CommunityEvent.filter({
        status: 'published'
      });
      ...
      const scoredEvents = upcomingEvents.map(event => {
        ...
        const daysUntil = (new Date(event.event_date) - now) / (1000 * 60 * 60 * 24);
                ...
                <Card className="bg-white hover:shadow-lg transition-all border-2 border-purple-200">
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${categoryColors[event.category]} flex items-center justify-center flex-shrink-0`}>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          <Badge className={`bg-gradient-to-r ${categoryColors[event.category]} text-white text-xs`}>
