...
// Adjust the import path for base44 based on your project structure.
...
// Balloon Component for Milestone Celebrations
...
export default function CountingFun({ onComplete }) {
  ...
  // utteranceRef is no longer needed as utterances are created locally within speakNext/fallbackToBrowserTTS
  const speechQueueRef = useRef([]);
  const isSpeakingRef = useRef(false);

  // --- TTS Function with Queue System and Volume Control ---
    ...
    const tryElevenLabs = async () => {
      try {
        const response = await base44.functions.invoke('generateStandardSpeech', {
          ...
          style: isLoud ? 0.8 : (isEmphatic ? 0.6 : 0.5),
          use_speaker_boost: true
        });

        // Check if response indicates fallback is needed
        ...
        // console.warn('ElevenLabs error, falling back to browser TTS:', error); // Keeping this for dev visibility
      }
  ...
  const speak = useCallback((text, options = {}) => {
    ...
    try {
      ...
      console.warn('Error queueing speech, continuing without audio:', error);
    }
  ...
  useEffect(() => {
    const handleVoicesChanged = () => {
      try {
        ...
        if (voices.length === 0) {
          ...
          console.log('Browser speech synthesis voices loaded:', voices.length);
        }
      } catch (error) {
        console.warn('Error loading voices in handleVoicesChanged:', error);
      }
    };
    ...
    if ('speechSynthesis' in window) {
      ...
      console.warn('Browser speech synthesis not supported in this browser.');
    }
    ...
    return () => {
      try {
        ...
        console.warn('Error cleaning up speech synthesis event listener:', error);
      }
  ...
  // Activate voice on first user interaction
  const activateVoice = useCallback(() => {
    if (!isSpeechSupported || !needsActivation) return;

    try {
      // Test speech to activate using browser TTS for initial activation simplicity
      // and because ElevenLabs might add latency for a quick activation UX.
      ...
      testUtterance.lang = 'en-US'; // Ensure language is set for activation too
      testUtterance.onend = () => {
        setNeedsActivation(false); // Only set to false after test speech finishes
      };
      testUtterance.onerror = (event) => {
        console.warn('Error activating voice (browser TTS):', event.error);
        setNeedsActivation(false); // Still allow activation even if test speech fails
      };
      window.speechSynthesis.speak(testUtterance);
      playSound('click');
    } catch (error) {
      console.warn('Error initiating browser speech for activation:', error);
      setNeedsActivation(false); // Also set to false if there's an immediate error
    }
  }, [isSpeechSupported, playSound, needsActivation]);

  // Check if current level is a milestone (5, 10, 15, 20) - wrapped in useCallback
  ...
  const setupLevel = useCallback((level) => {
    ...
    // Select item based on level, cycling through available items
    const itemIndex = (level - 1) % countingItems.length;
    const newItem = countingItems[itemIndex];
    setCurrentItem(newItem);

    // Create grid with more items for higher levels, ensuring we have enough
    ...
    // Clear speech queue for new level
    speechQueueRef.current = [];
    isSpeakingRef.current = false;
    try {
      window.speechSynthesis.cancel();
    } catch (error) {
      console.warn('Error cancelling speech in setupLevel:', error);
    }

    // Special announcement for milestones, only if voice is active
    if (!needsActivation) {
  ...
  const handleItemTouch = (itemId) => {
    // Activate voice on first touch if not already active
    if (needsActivation) {
      activateVoice();
      // Small delay before speaking the current level's instructions after activation
      setTimeout(() => {
      ...
      }, 500); // Give a brief moment for the activation speech to start/finish
      // It's crucial to return here and let the next touch handle the actual counting if activation needed delay.
      // Or, alternatively, proceed with the touch, but ensure the speak calls are queued properly.
      // For simplicity, we'll let it process the touch immediately but the 'speak' for level will be delayed.
    }
    ...
    // AI COUNTS ALONG! Make the last number LOUD and emphatic!
    if (isLastNumber) {
      speak(`${currentNumber}!`, { isLoud: true });

      // Add congratulations right after the last number
      ...
      // Regular number - AI COUNTS ALONG with normal volume
      speak(`${currentNumber}`);
    }
  };
  ...
  const resetGame = () => {
    ...
    try {
      ...
      console.warn('Error cancelling speech in resetGame:', error);
    }

    setupLevel(1);
    setTotalCorrect(0);
    // When resetting, if voice was activated, it remains active.
    // If we want to re-show the activation banner, uncomment the line below.
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-gradient-to-br from-yellow-100 to-orange-100 rounded-xl min-h-[600px] relative overflow-hidden">
      ...
      {/* Instructions with Voice Indicator */}
