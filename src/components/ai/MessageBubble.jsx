...
const FunctionDisplay = ({ toolCall }) => {
    ...
    // Handle navigation if this is a navigateToPage function
    useEffect(() => {
        if (name === 'navigateToPage' && status === 'completed' && parsedResults?.success && parsedResults?.pageName) {
            const pageUrl = createPageUrl(parsedResults.pageName);
            console.log('🚀 Navigating to:', pageUrl);
            
            toast.success(`Taking you to ${parsedResults.pageName}...`);
            
            setTimeout(() => {
                window.location.href = pageUrl;
            }, 800);
        }
    }, [name, status, parsedResults]);
    
    const isError = results && (
        (typeof results === 'string' && /error|failed/i.test(results)) ||
        (parsedResults?.success === false && !parsedResults?.action)
    );
                    ...
                    <div className="relative group">
                        ...
                        {!isUser && message.content && (
                            <Button
                                ...
                                variant="ghost"
                                onClick={handleSpeak}
                                className="absolute -right-10 top-2 h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity"
